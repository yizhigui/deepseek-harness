...
5. Plugin changes stop the backend before modifying the current profile. Package failures restore the pre-mutation manifest and lockfile through the existing preparation path. A durable metadata snapshot lets startup retry restoration after a crash. Successful preparation commits before starting the Host; Host startup failure retains the committed graph. Desktop creates no staging profile or package-directory copy.
...
Kernel guards exclude concurrent package transactions and surviving pnpm workers; `$DSH_HOME/profiles/desktop/lock` records diagnostic PIDs and a transaction token. Recovery acquires both guards before replacing abandoned ownership. Live guards never expire. Legacy PID-only records are reclaimed only when the PID is confirmed absent; corrupt or unreadable ownership fails closed. The independent `desktop-packages-pending` marker requests preparation, while `desktop-transaction-snapshot.json` preserves interrupted mutation metadata until restoration or commit. Reset preserves its held lock. See the [recovery decision](../../.agents/notes/implemented/bug-fix/2026-09-21-desktop-package-lock-recovery.md).
